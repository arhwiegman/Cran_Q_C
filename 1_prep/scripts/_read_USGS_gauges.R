# this script summarizes discharge data from select USGS gauges 

# dependancies
library(tidyverse)

# set input directory
indir = file.path(getwd(),"0_raw/USGS/Gauges")
...
df_info = read_delim(file.path(indir,"_Info.txt"),
                        comment= c("#")) %>%
  .[2:nrow(.),] %>% 
...
for (fn in filenames){
  print(fn)
  df=read_delim(file.path(indir,fn),
...
df_MA_USGS_GUAGE_summary <- left_join(df_info,df_summary,by="site_no") 

# join the two tables and write the outputs to csv
df_MA_USGS_GUAGE_summary %>% write_csv('1_prep/data/df_MA_USGS_GUAGE_summary.csv')
