...
df_bogs <- df_bogs_raw %>% 
  ...
  select(OBJECTID,
         ...
         BOG_NAME,
         EMBAY_NAME,
         # Concentrations predicted by EBK model. 
         NO3_mgl_p25,
         NO3_mgl_p50,
         NO3_mgl_p75,
         TN_mgl_p25,
...
df_bogs_processed <- df_bogs %>%
  ...
  # calculating loads to cranberry bogs loading rate contributingh area method (MEP, L2)
  # note CCC and MEP_Buildout estimates not used. 
...
df_EMBAY_sums <- df_bogs_processed %>% 
  ...
  select(EMBAY_NAME,
         ...
         LR_kghay_MEP, # kg N ha-1 y-1, Annual Areal TN Loading Rate for Embayment estimated by MEP
